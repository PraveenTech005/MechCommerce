...
const UsersIndex = () => {
      ...
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              await axios.delete(
                `${process.env.EXPO_PUBLIC_API_SERVER}/user/${id}`,
                {
  ...
  const handleChangeRole = async (id, newRole) => {
    try {
      await axios.put(
        `${process.env.EXPO_PUBLIC_API_SERVER}/user/role/${id}`,
        { role: newRole },
        {
          headers: { Authorization: `Bearer ${user?.token}` },
        },
    ...
    <View className="bg-white mx-4 mb-4 p-5 rounded-xl shadow-sm border border-gray-100 flex-col">
      <View className="flex-row justify-between items-start mb-4">
        <View className="flex-1 mr-2">
          ...
          <Text className="text-gray-500 text-sm mt-1 flex-row items-center">
            <Ionicons name="calendar-outline" size={14} color="gray" /> Joined:{" "}
      ...
      <View className="flex-row items-center">
        ...
        <View className="flex-1">
          ...
          >
            <Text
              className={`font-semibold capitalize ${item.role === "admin" ? "text-purple-700" : "text-blue-700"}`}
            >
              {item.role}
            </Text>
            <Ionicons
              name={openDropdownId === item._id ? "chevron-up" : "chevron-down"}
              size={16}
              color={item.role === "admin" ? "#7e22ce" : "#1d4ed8"}
            />
          </TouchableOpacity>
        </View>
      </View>
      ...
      {openDropdownId === item._id && (
        <View className="mt-2 bg-white border border-gray-200 rounded-lg shadow-sm">
          {["admin", "user"].map((roleType) => (
            ...
            >
              <Text
                className={`text-center font-bold capitalize pt-1 ${roleType === "admin" ? "text-purple-700" : "text-blue-700"}`}
              >
                {roleType}
              </Text>
            </TouchableOpacity>

          ...
          onChangeText={(text) => {
            ...
            const searchResults = products.filter(
              (item) =>
                ...
                (item.vehicle && item.vehicle.map((v) => {
                  return typeof v.brand === 'object' ? (v.brand?.name || v.brand?.brand || "") : (v.brand || "");
                }).join(" ").toLowerCase().includes(lowerText)),
            );
      ...
      <View className="w-full flex-row flex-wrap justify-between gap-y-4 pb-28">
        {allproducts.map((item, i) => (
          <TouchableOpacity
            ...
            onPress={() => {
              const originalIndex = products.findIndex((p) => p._id === item._id);
              router.push({

...
const Login = () => {
  ...
  const handleLogin = async () => {
    ...
    if (!validateEmail(cred.email) || !validatePassword(cred.password)) {
      return Toast.show({
        type: "info",
        text1: "Invalid email or password format",
      });
    }

    try {
      setLoading(true);
      const res = await axios.post(`${process.env.EXPO_PUBLIC_API_SERVER}/user/login`, cred);
      ...
      >
        <View className="flex flex-1 items-center justify-center px-6">
          ...
          <View className="mb-8 w-[80%]">
            <Text className="text-4xl font-bold text-gray-900 mb-1 text-center">
              Welcome Back
            </Text>
          ...
          >
            ...
            <View>
              ...
              <TextInput
                value={cred.email}
                onChangeText={(t) => setCred({ ...cred, email: t })}
